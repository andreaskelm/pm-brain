...
Use this guide after important conversations or when tuning the agent. Answer: **Is [AGENTS.md](../../AGENTS.md) (and `../ORCHESTRATION.md`, `../EVALUATION.md`) guiding the user and answering correctly?** Use this for human transcript review; for automated regression, use `harness/run_scenario.py`. When you learn something new, use the "Where to update" map to edit the right file.
...
- [ ] **Cite:** Did it cite repo paths (e.g. `2-Methods/...`) and avoid generic advice?
- [ ] **Challenge:** Did it challenge assumptions when appropriate (e.g. "What evidence supports that?") without being condescending?
- [ ] **Meta:** Did it suggest a meta action when relevant (e.g. log in 5-Growth, forecast log, learning log, or "evolve rules")?
...
| If you see… | Edit… |
|-------------|--------|
| **Questioning weak / jumps to template** | [../../AGENTS.md](../../AGENTS.md) — "Think first, always"; [../coaching/prompts.md](../coaching/prompts.md); [../coaching/braindump.md](../coaching/braindump.md) |
| **Wrong or no framework suggested** | [../../2-Methods/0-template-finder.md](../../2-Methods/0-template-finder.md); [../ORCHESTRATION.md](../ORCHESTRATION.md); [../skills/pm-brain-workflow/SKILL.md](../skills/pm-brain-workflow/SKILL.md) |
| **Overbearing / does thinking for user** | [../../AGENTS.md](../../AGENTS.md) — "Challenge before validate", product_sense persona |
| **Misses product_sense vs execution** | [../../AGENTS.md](../../AGENTS.md) — "Routing Intent"; [../../docs/architecture.md](../../docs/architecture.md) — mode flow |
| **No meta / no 5-Growth suggestions** | [../../AGENTS.md](../../AGENTS.md) — meta_reflection persona; [../ORCHESTRATION.md](../ORCHESTRATION.md) — meta_reflection state |
| **Evaluation / quality checks not offered** | [../EVALUATION.md](../EVALUATION.md); framework folder READMEs (PRD, Opportunity Assessment, North Star, One-Pagers) |

---

## 4. Scenario-based review prompts

Use these when you have a real chat that matches a scenario type. For executable criteria, read that scenario's `expected.yaml` under `scenarios/behavior/` (index: [agent-behavior-scenarios.json](agent-behavior-scenarios.json)).

**Vague product idea** (e.g. "I want to build an app for remote teams") — `vague_product_idea_001`

- Did the agent ask about the specific problem being solved and who the target users are before suggesting a PRD or features?
...
- **"Is the recommended file change the right architectural home?"** — see "Where to update" map. `.cursor/rules/pm-brain.mdc` is the universal always-on layer in Cursor; don't route personal, org-specific, or procedural rules into it.
...
| Rule ended up in wrong file | Does the rule belong in a universal layer (pm-brain.mdc / AGENTS.md lenses) or a specific file (ORCHESTRATION.md, MEMORY.md, USER.md)→ |
