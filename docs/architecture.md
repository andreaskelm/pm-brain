...
- **Contradiction detection (decision-level):** When new info contradicts a logged decision, forecast, reopen trigger, or stated belief in repo files, the agent surfaces it in one sentence and checks stored reopen triggers in forecast-log and prioritization-decision-log. Belief-level stays conversational plus the weekly live-assumptions block.
- **Evidence strength:** When a load-bearing claim appears, the agent names its tier in passing (documented > verbal > hunch > industry). Vocabulary only — see [evidence-strength.md](../2-Methods/1-Foundations/1-Mental-Models/1-Decision-Making/7-evidence-strength.md).
