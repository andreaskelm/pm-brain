...
1. Load the **always-on bootstrap set**:
   ...
   - [USER.md](USER.md) (personal + work context)
...
**Token efficiency (cross-cutting):** Use the cheapest model tier that can do the job well. PM Brain's thinking and coaching conversations require a capable mid-tier or higher model — the golden rule and routing depend on strong instruction-following; fast/cheap models skip the braindump. BUT once hard thinking is done, mechanical work (subagents for repo exploration, link audits, bulk file ops; artifact production from completed thinking) can and should use a fast/cheap model. When spawning subagents via the Task tool, prefer `explore` for read-only exploration and `shell` for mechanical ops; use `generalPurpose` only when the subtask requires synthesis or judgment. When transitioning from product_sense to execution_mode on a mechanical artifact (filling a template, drafting from completed thinking), it is appropriate to note that the user could switch to a lighter model for that phase. If a subagent realizes it needs more capability than expected, return to the parent rather than silently escalating. Parent always owns final synthesis and coaching tone.
...
|---------|--------------|-------|
| **Conversation start (bootstrap)** | [AGENTS.md](AGENTS.md), [ORCHESTRATION.md](ORCHESTRATION.md), [.cursor/rules/voice.mdc](.cursor/rules/voice.mdc), [.cursor/rules/thinking.mdc](.cursor/rules/thinking.mdc), [USER.md](USER.md) | Mandatory. No response before these are loaded. Resumed sessions and attachment context do NOT substitute. |
...
| **Company/org context mentioned** | Start with [01-Company-Context/README.md](01-Company-Context/README.md), then relevant files per [MEMORY.md](MEMORY.md) wake table | Check filesystem before asking user. Consult [CONTEXT-HEALTH.md](01-Company-Context/CONTEXT-HEALTH.md) when docs feed into real decisions. |
