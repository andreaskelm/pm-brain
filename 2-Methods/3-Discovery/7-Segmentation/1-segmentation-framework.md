...
- User only needs prioritization between known options (use 2-Strategic-Execution/4-Prioritization)
- User has no job research yet (use JTBD first)
- User is optimizing an existing product by usage patterns only (mention behavioral segmentation briefly; see 6-Metrics for usage/cohort analysis)
...
- You're only optimizing by usage (e.g. power vs casual users); use behavioral signals and 6-Metrics; mention in framework below
- You need one-off prioritization between initiatives (use 2-Strategic-Execution/4-Prioritization)

## The Process

### 1. Job clustering (from JTBD stories to 3–5 segments)

- **Input:** Job stories from JTBD (`3-Jobs-To-Be-Done`) — When / I want to / So I can, plus forces and context.
- **Steps:**
...
- Create 1–2 personas **per priority segment** using 5-Personas. Same segment can have multiple personas (e.g. different channels, copy tone); same persona type can appear in different segments when the job changes.
- **Workflow:** Define segments (this framework) → Prioritize which 1–2 segments to serve (2-Strategic-Execution/4-Prioritization) → Create personas for those segments (5-Personas).

## Connection to Other Frameworks

| Framework | Role |
|-----------|------|
| **JTBD (`3-Jobs-To-Be-Done`)** | Provides job stories and forces; segmentation clusters those jobs into segments. |
| **Problem-Solution Space (`6-Problem-Solution-Space`)** | Validates which problems matter to which segments; different segments can have different problem priorities. |
| **Personas (`5-Personas`)** | Personas are derived from priority segments; segments define *what* to build, personas define *how* to execute. |
| **PMF (`8-Product-Market-Fit`)** | Assess PMF by segment when possible; one segment may have strong PMF while another does not. |
| **Prioritization (`2-Strategic-Execution/4-Prioritization`)** | Use to choose which 1–2 segments to serve first; don't invent a separate segment-prioritization system. |
...
- **Format:** Markdown (`.md`)
- **Location:** `4-Research/segments/` or initiative folder
- **Filename:** `segment-[segment-name]-[YYYY-MM-DD].md` or one doc with all segments
- **Handoffs:** JTBD (`3-Jobs-To-Be-Done`) → this framework → Prioritization (`2-Strategic-Execution/4-Prioritization`) for which segment(s) to serve → Personas (`5-Personas`) for priority segments → PMF (`8-Product-Market-Fit`) to assess fit by segment.

## Links

- JTBD: [3-Jobs-To-Be-Done](../3-Jobs-To-Be-Done/README.md)
- Problem-Solution Space: [6-Problem-Solution-Space](../6-Problem-Solution-Space/README.md)
- Personas: [5-Personas](../../4-Execution/5-Personas/README.md)
- PMF: [8-Product-Market-Fit](../8-Product-Market-Fit/README.md)
- Prioritization: [2-Strategic-Execution/4-Prioritization](../../2-Strategy/2-Strategic-Execution/4-Prioritization/)
