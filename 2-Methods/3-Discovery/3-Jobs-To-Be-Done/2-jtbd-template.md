# JTBD Job File Template

Use this when creating a single job file (one per core job). Copy this into `4-Research/jobs/` (or an initiative folder), rename it to `job-[concise-job-handle]-[YYYY-MM-DD].md`, and fill it in based on your JTBD research.
...
*For systematic segmentation:* Once you have many job stories, use **7-Segmentation** to cluster jobs into 3–5 market segments (job-based, not demographic), define and validate them, then prioritize which to serve. See [7-Segmentation](../7-Segmentation/README.md).
