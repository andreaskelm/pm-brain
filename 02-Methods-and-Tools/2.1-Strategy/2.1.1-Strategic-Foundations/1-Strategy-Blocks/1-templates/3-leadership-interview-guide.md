# Leadership Interview Guide

### Pre-Interview Preparation

**Logistics:**
- Book 60-90 minutes (45 min interview + 15 min buffer)
- Send agenda 2 days prior with context
- Choose quiet location or high-quality video setup
- Bring notebook for capturing verbatim quotes

**Context Setting Email Template:**
```
Subject: Strategy Development - Leadership Perspective Session

Hi [Name],

We're developing our 2-year product strategy and would love your insights. This conversation will help us understand what success looks like from leadership's perspective and inform our strategic pillar selection.

Agenda (60 minutes):
- Your vision of success and failure
- Key success measures and principles
- Learning from past experiences  
- Ideas and opportunities you're excited about

No preparation needed - just your thoughts and perspective.

Best,
[Your name]
```

### Interview Protocol (60 minutes)

**Opening (5 minutes)**
"We're in the early stages of developing our product strategy. I'd love to understand your perspective on where we should focus our energy over the next 2 years to have maximum impact."

#### Section 1: Success & Failure Vision (15 minutes)

**Primary Questions:**
1. **"If you could fast-forward 2 years and we've had wild success, what does that look like?"**
   - Follow-up: "What specific behaviors would you see from users/customers?"
   - Follow-up: "How would our business metrics be different?"

2. **"On the flip side, what would failure look like? What keeps you up at night?"**
   - Follow-up: "What warning signs would tell us we're heading in the wrong direction?"

3. **"What's the most exciting version of our success that you can imagine?"**
   - Follow-up: "What would have to be true for that to happen?"

**Interviewer Notes:**
- Capture exact language they use for success/failure
- Note emotional emphasis and energy shifts
- Ask for concrete examples when they speak abstractly

#### Section 2: Success Measures (10 minutes)

**Primary Questions:**
4. **"What are the most important metrics of success for us?"**
   - Follow-up: "How would you prioritize these if you had to rank them?"

5. **"How would you know we're winning against competitors?"**
   - Follow-up: "What leading indicators would give you confidence we're on track?"

6. **"What metrics should we NOT optimize for?"**
   - Follow-up: "What would be dangerous to focus on?"

**Interviewer Notes:**
- Distinguish between lagging indicators (revenue) and leading indicators (engagement)
- Note any tension between different metrics they mention

#### Section 3: Guiding Principles (10 minutes)

**Primary Questions:**
7. **"What principles should guide our product decisions over the next 2 years?"**
   - Follow-up: "Can you give me an example of when this principle helped make a tough decision?"

8. **"What should we never compromise on, even under pressure?"**
   - Follow-up: "How do we maintain this when facing resource constraints?"

9. **"What makes us unique? What's our secret sauce?"**
   - Follow-up: "How do we leverage this advantage in our strategy?"

#### Section 4: Learning from Past (10 minutes)

**Primary Questions:**
10. **"Why haven't things worked as expected in the past?"**
    - Follow-up: "What patterns do you see in our misses vs. hits?"

11. **"If you could go back and change our approach to product development, what would you do differently?"**
    - Follow-up: "What did we learn that we can apply going forward?"

12. **"What assumptions have we made that turned out to be wrong?"**
    - Follow-up: "How do we avoid similar blind spots this time?"

#### Section 5: Ideas & Vision (10 minutes)

**Primary Questions:**
13. **"What ideas are you most excited about that we should consider?"**
    - Follow-up: "What makes this idea compelling to you?"

14. **"What's your most contrarian view about our space?"**
    - Follow-up: "Why do you believe this when others might disagree?"

15. **"If we had unlimited resources, what would you want us to build?"**
    - Follow-up: "What's the smallest version of that we could test?"

**Closing (5 minutes)**
16. **"What questions should I have asked that I didn't?"**
17. **"Who else should I talk to for a different perspective?"**
18. **"What's the most important thing for me to remember from our conversation?"**

### Post-Interview Documentation Template

```markdown
# Leadership Interview Notes

**Interviewee:** [Name, Role]
**Date:** [Date]
**Interviewer:** [Your name]
**Duration:** [Actual time]

## Key Quotes (Verbatim)
- Success: "[exact quote about success vision]"
- Failure: "[exact quote about failure concerns]"
- Principles: "[exact quote about guiding principles]"
- Past Learning: "[exact quote about what didn't work]"
- Excitement: "[exact quote about ideas/opportunities]"

## Success Definition
**Their Vision of Success:**
- [summarize their 2-year success picture]

**Success Metrics They Care About:**
1. [primary metric with their reasoning]
2. [secondary metric with their reasoning]
3. [other metrics mentioned]

## Failure Concerns
**What Keeps Them Up at Night:**
- [primary concern with context]
- [secondary concerns]

**Warning Signs They'd Watch For:**
- [early indicators of problems]

## Guiding Principles
**Decision-Making Principles:**
- [principle 1 with example if provided]
- [principle 2 with example if provided]

**Non-Negotiables:**
- [what they won't compromise on]

## Learning from Past
**Why Things Haven't Worked:**
- [pattern 1 with specific examples]
- [pattern 2 with context]

**Key Insights:**
- [what they learned that applies to strategy]

## Ideas & Opportunities
**Most Exciting Ideas:**
- [idea 1 with their reasoning]
- [idea 2 with context]

**Contrarian Views:**
- [their unique perspective on the market/product]

## Surprises & Insights
**Unexpected Findings:**
- [things that surprised you in their responses]
- [new perspectives you hadn't considered]

## Strategic Implications
**For Pillar Selection:**
- [how their input should influence strategy]

**For Success Measurement:**
- [metrics/approaches they validated or challenged]
```

### Interview Tips for Success

**Before the Interview:**
- Research their background and current focus areas
- Review recent company communications they've authored
- Prepare follow-up questions based on their likely interests

**During the Interview:**
- Write down exact quotes—their language matters for strategy communication
- Notice what energizes them vs. what concerns them
- Ask "why" and "how" to get deeper than surface-level answers
- Be comfortable with silence—let them think

**After the Interview:**
- Document within 24 hours while memory is fresh
- Share draft notes with interviewee for accuracy (optional)
- Look for patterns across multiple leadership interviews


